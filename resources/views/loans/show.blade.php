<x-app-layout>
    ...
    @can('disburse', $loan)
        <form method="POST" action="{{ route('loans.disburse', $loan) }}" class="mt-6 rounded-2xl border border-blue-200 bg-blue-50 p-5">@csrf<h2 class="font-bold text-blue-900">Pencairan Pinjaman</h2><p class="mt-1 text-sm text-blue-700">Pencairan mengaktifkan sisa pokok sebesar Rp {{ number_format($loan->principal_amount, 0, ',', '.') }}.</p><div class="mt-4 grid gap-4 md:grid-cols-2"><label><span class="text-sm font-medium">Tanggal pencairan</span><input type="date" name="disbursed_at" value="{{ now()->format('Y-m-d') }}" min="{{ $loan->approved_at?->format('Y-m-d') }}" max="{{ now()->format('Y-m-d') }}" class="mt-1 w-full rounded-xl border-blue-200" required></label><label><span class="text-sm font-medium">Catatan pencairan</span><input name="disbursement_notes" maxlength="2000" class="mt-1 w-full rounded-xl border-blue-200"></label></div><button class="mt-4 rounded-xl bg-blue-700 px-5 py-2 font-semibold text-white">Konfirmasi Pencairan</button></form>
    @endcan

    @can('create', [App\Models\Installment::class, $loan])
        <div class="mt-6 flex justify-end"><a href="{{ route('installments.create', $loan) }}" class="rounded-xl bg-emerald-700 px-5 py-3 font-semibold text-white">Catat Pembayaran Angsuran</a></div>
    @endcan

    @if($loan->installments->isNotEmpty())
        <div class="mt-6 rounded-2xl bg-white p-5 shadow-sm"><h2 class="font-bold">Riwayat Angsuran</h2><div class="mt-4 overflow-x-auto"><table class="w-full text-sm"><thead class="text-left text-slate-500"><tr><th class="py-3">Ke</th><th>Tanggal</th><th>Nomor</th><th class="text-right">Pokok</th><th class="text-right">Bunga</th><th class="text-right">Denda</th><th class="text-right">Total</th><th></th></tr></thead><tbody>@foreach($loan->installments->sortByDesc('installment_number') as $payment)<tr class="border-t"><td class="py-3">{{ $payment->installment_number }}</td><td>{{ $payment->paid_at->format('d-m-Y') }}</td><td class="font-mono text-xs">{{ $payment->payment_number }}</td><td class="text-right">Rp {{ number_format($payment->principal_paid, 0, ',', '.') }}</td><td class="text-right">Rp {{ number_format($payment->interest_paid, 0, ',', '.') }}</td><td class="text-right">Rp {{ number_format($payment->penalty, 0, ',', '.') }}</td><td class="text-right font-bold">Rp {{ number_format($payment->total_paid, 0, ',', '.') }}</td><td class="text-right"><a href="{{ route('installments.show', $payment) }}" class="font-semibold text-blue-700">Bukti</a></td></tr>@endforeach</tbody></table></div></div>
    @endif
